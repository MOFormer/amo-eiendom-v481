...
st.title("AMO Eiendom v49.2.16 – Automatisk lagring og 'Ny eiendom'-knapp")
...
def nullstill_alle():
    for k in felt_navn:
        st.session_state[k] = "" if isinstance(felt_navn[k], str) else 0.0
...
with st.sidebar:
    ...
    if st.session_state.eiendommer:
        valgt = st.selectbox("Velg eiendom", list(st.session_state.eiendommer.keys()), key="valgt_eiendom_drop")
